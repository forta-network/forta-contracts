    ...
    describe('Scanners', function () {
        ...
        it('happy path', async function () {
            ...
            await expect(this.scanners.connect(this.accounts.manager).setStakeThreshold({ max: '500', min: '100', activated: true }, 1))
                .to.emit(this.scanners, 'StakeThresholdChanged')
                .withArgs('1', '100', '500', true);
        ...
        it('cannot stake without max cap set', async function () {
            ...
            await expect(this.staking.connect(this.accounts.user1).deposit(subjectType1, subjectInUnititializedChain, '101')).to.be.revertedWith(
                'StakeInactiveOrSubjectNotFound()'
            );
        });

        it('cannot set unauthorized', async function () {
            await expect(this.scanners.connect(this.accounts.user1).setStakeThreshold({ max: '500', min: '100', activated: true }, 1)).to.be.revertedWith(
                `MissingRole("${this.roles.SCANNER_ADMIN}", "${this.accounts.user1.address}")`
            );
        });

        it('cannot set min > max', async function () {
            await expect(this.scanners.connect(this.accounts.manager).setStakeThreshold({ max: '50', min: '100', activated: true }, 1)).to.be.revertedWith(
