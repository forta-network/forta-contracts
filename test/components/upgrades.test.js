...
describe.skip('Upgrades testing', function () {
        ...
        it(' 0.1.1 -> 0.1.5', async function () {
            ...
            agents = await upgrades.upgradeProxy(agents.address, AgentRegistry_0_1_2, {
                call: {
                    ...
                    args: [this.contracts.subjectHandler.address],
                },
                constructorArgs: [this.contracts.forwarder.address],
                unsafeAllow: ['delegatecall'],
            ...
            await agents.connect(this.accounts.user1).disableAgent(AGENT_ID, 1);
            expect(await agents.getSubjectHandler()).to.be.equal(this.contracts.subjectHandler.address);
    ...
    describe('Scanner Registry', async function () {
        it(' 0.1.0 -> 0.1.4', async function () {
            ...
            await this.contracts.subjectHandler.setStakeSubject(0, scannerRegistry.address);
            ...
            expect(await scannerRegistry.getSubjectHandler()).to.be.equal(this.contracts.subjectHandler.address);
    ...
    describe('FortaStaking', async function () {
        it('0.1.0 -> 0.1.1', async function () {
            ...
            const StakeSubjectHandler_0_1_0 = await ethers.getContractFactory('StakeSubjectHandler_0_1_0');
            this.subjectHandler = await upgrades.deployProxy(StakeSubjectHandler_0_1_0, [this.access.address, mockRouter.address, this.staking.address], {
                kind: 'uups',
                constructorArgs: [this.contracts.forwarder.address],
                unsafeAllow: ['delegatecall'],
            });
            await this.subjectHandler.deployed();
            ...
            await originalScanners.deployed();
            await this.subjectHandler.setStakeSubject(0, this.scanners.address);
            await this.subjectHandler.setStakeSubject(1, this.agents.address);
            ...
            await this.staking.setSubjectHandler(this.subjectHandler.address);
