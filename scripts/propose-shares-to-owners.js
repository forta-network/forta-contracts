...
async function transferShares(config = {}) {
    ...
    const shareBalances = await contracts.staking.balanceOfBatch(excludingOurOwn.map(x => x.owner), excludingOurOwn.map(x => x.activeShareId));
    ...
    const results = await Promise.all(
        toSend.map(x => {
            return {
                ...x,
                transferCalldata: contracts.staking.interface.encodeFunctionData(
                    'safeTransferFrom',
                    [MULTISIG, x.owner, x.activeShareId, AMOUNT, ethers.constants.HashZero]
                )
            }
        })
        .chunk(chunkSize)
