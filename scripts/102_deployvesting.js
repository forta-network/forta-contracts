...
async function main() {
    ...
    if (!(chainType & loadEnv.CHAIN_TYPE.ROOT)) { throw 'vesting should only be deployed on root chains'; }
    ...
    const vesting = await ethers.getContractFactory('VestingWalletV2', deployer).then(factory => utils.tryFetchProxy(
        ...
        { unsafeAllow: 'delegatecall', constructorArgs: [ RootChainManager, L2EscrowFactory, L2EscrowTemplate ] },
    ));
