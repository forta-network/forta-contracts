...
async function main() {
    ...
    const balancesQuery = (await Promise.all(
        toSendFort.chunk(CHUNK_SIZE)
        .map(chunk => {

            return contracts.relayer.callStatic.relay(contracts.forta.address, chunk.map(x => x.balanceCall))
        })
    )).flat()
    console.log('balancesQuery:', balancesQuery.length)
    
    const notSentYet = _.zip(toSendFort, balancesQuery).map(x => {
        return {
            ...x[0],
    ...
    console.log('Sent!');
    const resultBalancesQuery = (await Promise.all(
        toSendFort.chunk(CHUNK_SIZE)
        .map(chunk => {
            return contracts.relayer.callStatic.relay(contracts.forta.address, chunk.map(x => x.balanceCall))
        })
