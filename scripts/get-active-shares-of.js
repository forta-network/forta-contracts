...
async function transferShares(config = {}) {
    ...
    const data = mintings
        ...
        .map(registryId => { return { registryId: registryId, activeShareId: stakingUtils.subjectToActive(SUBJECT_TYPE, registryId)}})
        .map(item => { return { registryId: item.registryId, activeShareId: item.activeShareId.toString(), call: contracts.staking.interface.encodeFunctionData('balanceOf', [ADDRESS, item.activeShareId])}})
    const idChunks = []
