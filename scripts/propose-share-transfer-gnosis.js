...
async function transferShares(config = {}) {
    ...
    const deservingOfShares = rewardables.filter((x) => BigNumber.from(x.shares).gt(BigNumber.from('0')));
    fs.writeFileSync('./scripts/data/shares_week4.json', JSON.stringify(deservingOfShares));
