...
const rewardables = require('./data/not_sent_week3.json').filter(x => x.owner !== '0xDF21A5b1614D406a437A687b3fCCAE71d470436F');
...
async function getLastSent(lastBlock=END_BLOCK, tokenAddress, senderAddress) {
    const transfers = await axios.get(`https://api.polygonscan.com/api?module=account&action=tokentx&contractaddress=${tokenAddress}&address=${senderAddress}&startblock=${START_BLOCK}&endblock=${lastBlock}&page=1&offset=0&sort=asc&apikey=${process.env.POLYSCAN}`)
