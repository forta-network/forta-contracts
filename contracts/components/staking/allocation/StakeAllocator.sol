...
contract StakeAllocator is BaseComponentUpgradeable, SubjectTypeValidator, IStakeAllocator {
     ...
     */
    function allocateOwnStake(uint8 subjectType, uint256 subject, uint256 amount) external onlyAgencyType(subjectType, SubjectStakeAgency.DELEGATED) {
     ...
     */
    function unallocateOwnStake(uint8 subjectType, uint256 subject, uint256 amount) external onlyAgencyType(subjectType, SubjectStakeAgency.DELEGATED) {
        if (!_subjectHandler.canManageAllocation(subjectType, subject, _msgSender())) revert SenderCannotAllocateFor(subjectType, subject);
     ...
     * @param amount amount of stake to move from unallocated to allocated.
     */
    function allocateDelegatorStake(uint8 subjectType, uint256 subject, uint256 amount) external onlyAgencyType(subjectType, SubjectStakeAgency.DELEGATED) {
        if (!_subjectHandler.canManageAllocation(subjectType, subject, _msgSender())) revert SenderCannotAllocateFor(subjectType, subject);
     ...
     */
    function unallocateDelegatorStake(uint8 subjectType, uint256 subject, uint256 amount) external onlyAgencyType(subjectType, SubjectStakeAgency.DELEGATED) {
        if (!_subjectHandler.canManageAllocation(subjectType, subject, _msgSender())) revert SenderCannotAllocateFor(subjectType, subject);
     ...
     * @param amount amount of staked token.
     */
    function _allocateStake(uint8 subjectType, uint256 subject, uint256 amount) private {
        ...
        (int256 extra, uint256 max) = _allocationIncreaseChecks(subjectType, subject, SubjectStakeAgency.DELEGATED, amount);
     ...
     * @param amount amount of staked token.
     */
    function _unallocateStake(uint8 subjectType, uint256 subject, uint256 amount) private {
        uint256 activeSharesId = FortaStakingUtils.subjectToActive(subjectType, subject);
     ...
     * @param amount amount of incoming staked token.
     */
    function withdrawAllocation(uint256 activeSharesId, uint8 subjectType, uint256 subject, uint256 amount) external {
     ...
     * @param amount amount of incoming staked token.
     */
    function depositAllocation(uint256 activeSharesId, uint8 subjectType, uint256 subject, uint256 amount) external override {
        ...
        (int256 extra, ) = _allocationIncreaseChecks(subjectType, subject, agency, amount);
        if (extra > 0) {
     ...
     */
    function _allocationIncreaseChecks(uint8 subjectType, uint256 subject, SubjectStakeAgency agency, uint256 amount) private view returns (int256 extra, uint256 max) {
        uint256 subjects = 0;
