...
library EpochCheckpoints {
    ...
    function push(History storage self, uint256 value) internal returns (uint256, uint256) {
        ...
        if (pos > 0 && self._checkpoints[pos - 1]._epochNumber == getEpochNumber()) {
            self._checkpoints[pos - 1]._value = SafeCast.toUint224(value);
        } else {
            self._checkpoints.push(
                Checkpoint({_epochNumber: SafeCast.toUint32(getEpochNumber()), _value: SafeCast.toUint224(value)})
            );
        }
        return (old, value);
    }
