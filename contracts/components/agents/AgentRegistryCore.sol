...
abstract contract AgentRegistryCore is BaseComponentUpgradeable, FrontRunningProtection, ERC721Upgradeable, DirectStakeSubjectUpgradeable {
     ...
     * Also invokes _agentBypassesAgentUnitsRequirement and possibly _activeAgentUnitsBalanceUpdate.
    ...
    function registerAgent(
        ...
        bool _agentBypassesAgentUnitsReq = _agentBypassesAgentUnitsRequirement(msgSender, agentId, _agentUnits);
        if (!_agentBypassesAgentUnitsReq) { _activeAgentUnitsBalanceUpdate(msgSender, agentId, _agentUnits, AgentModification.Create); }
        _mint(msgSender, agentId);
        _beforeAgentUpdate(agentId, metadata, chainIds, redundancy, shards);
        _agentUpdate(agentId, metadata, chainIds, redundancy, shards);
        _afterAgentUpdate(agentId, metadata, chainIds, redundancy, shards);
    }
    ...
    /**
     * @notice Updates parameters of an agent (metadata, image, chain IDs...) if called by the agent owner.
    ...
    function updateAgent(uint256 agentId, string calldata metadata, uint256[] calldata chainIds, uint8 redundancy, uint8 shards) public onlyOwnerOf(agentId) onlySorted(chainIds) {
        ...
        uint256 _agentUnits = calculateAgentUnitsNeeded(chainIds.length, redundancy, shards);
        bool _agentBypassesAgentUnitsReq = _agentBypassesAgentUnitsRequirement(msgSender, agentId, _agentUnits);
        if (!_agentBypassesAgentUnitsReq) { _activeAgentUnitsBalanceUpdate(msgSender, agentId, _agentUnits, AgentModification.Update); }
        _beforeAgentUpdate(agentId, metadata, chainIds, redundancy, shards);
        _agentUpdate(agentId, metadata, chainIds, redundancy, shards);
        _afterAgentUpdate(agentId, metadata, chainIds, redundancy, shards);
    }
    ...
    */
     ...
     * Will check if certain requirements are met, and if so,
     * agent will not need agent units allocated to it.
     ...
     */
    function _beforeAgentUpdate(uint256 agentId, string memory newMetadata, uint256[] calldata newChainIds, uint8 newRedundancy, uint8 newShards) internal virtual {}
